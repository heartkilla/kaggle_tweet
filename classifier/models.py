...
class TweetModel(transformers.BertPreTrainedModel):
    def __init__(self, conf):
        ...
        self.clf = torch.nn.Linear(config.HIDDEN_SIZE * 2, 1)
    ...
    def forward(self, ids, mask, token_type_ids):
        ...
        clf_logits = torch.mean(torch.stack([
            self.clf(self.high_dropout(out[:, 0, :]))
            for _ in range(5)
        ], dim=0), dim=0)
        ...
        return start_logits, end_logits, clf_logits
