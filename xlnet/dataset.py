...
def process_data(tweet, selected_text, sentiment,
    ...
    input_ids_original, tweet_offsets = tokenizer.encode(tweet)
    ...
    # Input for XLNet
    input_ids = [3] + [sentiment_id[sentiment]] + [4] + \
                input_ids_original + [4]
    # No token types in XLNet
    ...
    if padding_len > 0:
        input_ids = input_ids + ([5] * padding_len)
...
class TweetDataset:
    def __init__(self, tweets, sentiments, selected_texts):
        ...
        self.tokenizer = SentencePieceTokenizer(f'{config.TOKENIZER_PATH}/spiece.model')
